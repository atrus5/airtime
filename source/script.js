...
document.addEventListener('DOMContentLoaded', function() {
    ...
    categoryBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            favoritesBtn.classList.remove('text-indigo-600');
            favoritesBtn.classList.add('text-gray-500');
    ...
    favoritesBtn.addEventListener('click', () => {
        ...
        filterTimers();
        categoryBtns.forEach(b => b.classList.remove('ring-2', 'ring-indigo-500'));
        favoritesBtn.classList.add('text-indigo-600');
        favoritesBtn.classList.remove('text-gray-500');
    ...
    function renderFilteredTimers(filteredTimers) {
         if (filteredTimers.length === 0) {
            timerContainer.innerHTML = `<div class="text-center py-8 text-gray-500" id="no-timers-message"><p>${showingFavorites ? 'No favorites found.' : 'No matching timers found.'}</p></div>`;
            return;
        }
        ...
        filteredTimers.forEach(timer => {
            ...
            switch(timer.category) {
                case 'beef': categoryColor = 'red'; break;
                case 'chicken': categoryColor = 'orange'; break;
                case 'fish': categoryColor = 'blue'; break;
                case 'pork': categoryColor = 'pink'; break;
                case 'vegetables': categoryColor = 'green'; break;
                case 'frozen': categoryColor = 'yellow'; break;
                default: categoryColor = 'gray';
            }
            const card = document.createElement('div');
            card.className = 'card bg-white rounded-lg shadow overflow-hidden border border-gray-100';
            card.innerHTML = `
                ...
                <div class="p-4">
                    <div class="flex justify-between items-start">
                        <div>
                            <h3 class="font-bold text-lg text-gray-800">${timer.name}</h3>
                            <div class="flex items-center mt-1 space-x-2">
                                <span class="px-2 py-1 bg-${categoryColor}-100 text-${categoryColor}-800 text-xs rounded-full">${timer.category.charAt(0).toUpperCase() + timer.category.slice(1)}</span>
                                <span class="px-2 py-1 bg-${timer.foodState === 'frozen' ? 'blue' : 'green'}-100 text-${timer.foodState === 'frozen' ? 'blue' : 'green'}-800 text-xs rounded-full">${timer.foodState.charAt(0).toUpperCase() + timer.foodState.slice(1)}</span>
                            </div>
                        </div>
                        <div class="flex flex-col items-end">
                            <div class="text-indigo-600 font-bold">${timer.temperature}°${timer.tempUnit}</div>
                            <div class="text-gray-500 text-sm">${timeString}</div>
                        </div>
                    </div>
                    ${timer.notes ? `<p class="text-gray-600 text-sm mt-2 border-t border-gray-100 pt-2">${timer.notes}</p>` : ''}
                    <div class="flex justify-end items-center mt-3">
                        <button class="text-indigo-500 mr-2 start-main-timer" title="Start this timer" data-minutes="${timer.minutes}" data-seconds="${timer.seconds || 0}"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" /></svg></button>
                        <button class="text-gray-500 mr-2 favorite-timer" data-id="${timer.id}" title="Favorite"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ${timer.favorite ? 'text-yellow-500 fill-current' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg></button>
                        <button class="text-gray-500 mr-2 edit-timer" data-id="${timer.id}" title="Edit"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                        <button class="text-red-500 delete-timer" data-id="${timer.id}" title="Delete"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                    </div>
                </div>
            `;
    ...
    async function toggleFavorite(id) {
        ...
        if (currentUser) {
            try {
                await db.collection('users').doc(currentUser.uid).collection('timers').doc(id.toString()).update({ favorite: newFavoriteState });
            } catch(err) { showToast(`Error: ${err.message}`, 'error'); }
        } else {
    ...
    function loadFromLocalStorage() {
        ...
        if (savedTimers) {
            ...
            timers = getSampleTimers(); // Load sample data for new users
        }
        renderTimers();
    }
    ...
    // Initial load
    // The auth state change listener will trigger the initial data load.
});
